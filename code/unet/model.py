...
class CAE_3D(object):
        ...
        for epoch in xrange(args.epoch):
            ...
            for idx in xrange(0, batch_idxs):
                ...
                if np.mod(counter, self.X_train.shape[0]) == 1:
                    print("Epoch %d - Loss %4.4f " % (epoch, (loss_sum / loss_count)));
                    loss_sum = 0;
                    loss_count = 0;
                    sys.stdout.flush();
    ...
    def generator(self, image):
        with tf.variable_scope("generator") as scope:
            ...
            e4 = conv3d(lrelu(e3), self.gf_dim*4, d_d=1, d_h=2, d_w=2, name='g_e4_conv');
            e5 = conv3d(lrelu(e4), self.gf_dim*4, name='g_e5_conv');
            ...
            self.d2, self.d2_w, self.d2_b = deconv3d(tf.nn.relu(d1), [self.batch_size, s_d_16, s_h_32, s_w_32, self.gf_dim*4], name='g_d2', with_w=True);
            d2 = tf.nn.dropout(self.d2, 0.5);
            print "d2, e4 ", d2.get_shape(), e4.get_shape();
            #d2 = tf.concat([d2, e5], 4);

            self.d3, self.d3_w, self.d3_b = deconv3d(tf.nn.relu(d2), [self.batch_size, s_d_8, s_h_16, s_w_16, self.gf_dim*4], name='g_d3', with_w=True);
            d3 = tf.nn.dropout(self.d3, 0.5);
            #d3 = tf.concat([d3, e4], 4);

            self.d4, self.d4_w, self.d4_b = deconv3d(tf.nn.relu(d3), [self.batch_size, s_d_8, s_h_8, s_w_8, self.gf_dim*2], d_d=1, d_h=2, d_w=2, name='g_d4', with_w=True);
