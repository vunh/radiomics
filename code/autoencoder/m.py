...
class CAE_3D(object):
    ...
    def build_model(self):
        ...
        self.reconstruct_loss = tf.reduce_mean(tf.abs(self.reconstruct_flat - self.input_flat));
    ...
    def validate(self):
        pred_days = np.zeros(shape=(self.X_val.shape[0], 2), dtype=np.float32);
        gt_days = np.zeros(shape=(self.X_val.shape[0], 2), dtype=np.float32);
        ...
        for idx in xrange(0, batch_idxs):
            ...
            batch_clinical_num = self.clinical_num_val[idx*self.batch_size:(idx+1)*self.batch_size];
            batch_name = self.name_val[idx*self.batch_size:(idx+1)*self.batch_size];
            batch_groundtruths_raw = self.Y_val_raw[idx*self.batch_size:(idx+1)*self.batch_size];
            ...
            lay1, lay2, loss_value, normgroundtruth, pred_sur = self.sess.run([self.net['fc1'], self.net['output'], self.residual_loss, self.raw_groundtruth, self.pred_survival], \
                    feed_dict={self.input_image: batch_images, self.raw_groundtruth: batch_groundtruths, \
            ...
            #print "name ", batch_name;
            #print "sum of layer 1", np.sum(lay1.flatten());
            #print "sum of layer 2", lay2;
            #print "sum of clinical num", batch_clinical_num;
            #print "loss ", loss_value;
            #print "sum of certainty", np.sum(batch_certainty.flatten());
            #print "pre_sur ", pred_sur;
            pred_days[loss_count] = pred_sur;
            gt_days[loss_count] = normgroundtruth;
            gt_days_raw[loss_count] = batch_groundtruths_raw[0];
    ...
    def generator(self, image, clinical_num):
        with tf.variable_scope("generator") as scope:
            ...
            net['conv4'] = lrelu(conv3d(net['pool2'], self.gf_dim, name='g_conv4'));
            net['pool4'] = tf.layers.max_pooling3d(net['conv3'], pool_size=(2,2,2), strides=(2,2,2));

            net['deconv1'] = deconv3d(net['pool4'],   [self.batch_size, s_deconv_d*1, s_deconv_h*1, s_deconv_w*1, self.gf_dim], d_d=2, d_h=2, d_w=2, stddev=0.02, name="g_deconv1");
            print "deconv1 shape ", net['deconv1'].get_shape();
            net['deconv2'] = deconv3d(net['deconv1'], [self.batch_size, s_deconv_d*2, s_deconv_h*2, s_deconv_w*2, self.gf_dim], d_d=2, d_h=2, d_w=2, stddev=0.02, name="g_deconv2");
            print "deconv2 shape ", net['deconv2'].get_shape();
            net['deconv3'] = deconv3d(net['deconv2'], [self.batch_size, s_deconv_d*4, s_deconv_h*4, s_deconv_w*4, self.gf_dim], d_d=2, d_h=2, d_w=2, stddev=0.02, name="g_deconv3");
            print "deconv3 shape ", net['deconv3'].get_shape();
            net['deconv4'] = deconv3d(net['deconv3'], [self.batch_size, s_deconv_d*8, s_deconv_h*8, s_deconv_w*8, self.gf_dim], d_d=2, d_h=2, d_w=2, stddev=0.02, name="g_deconv4");
            print "deconv4 shape ", net['deconv4'].get_shape();
